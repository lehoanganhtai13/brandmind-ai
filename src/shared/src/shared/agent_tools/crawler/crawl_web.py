...
from shared.agent_tools.crawler.crawl4ai_client import (
    Crawl4AIPresets,
...
def scrape_web_content(
    ...
    Use this function to scrape web content in different ways based on the
    specified mode.

    Args:
        web_url (str): The URL to scrape
        mode (str): Extraction mode - "clean", "summary", or "relevant"
        query (str, optional): Query for relevant content filtering (required for
            "relevant" mode)
    ...
    try:
        ...
        if mode == "clean":
            ...
            result = client.fetch_clean_content(
                url=web_url, config=Crawl4AIPresets.balanced()
            )
            ...
            result = client.fetch_relevant_content(url=web_url, query=query)

        else:
            logger.error(
                f"Invalid mode: {mode}. Supported modes: clean, summary, relevant"
            )
            return ScrapeResult(url=web_url, content="")
        ...
        logger.info(
            f"Successfully scraped {web_url} with mode '{mode}' - "
        ...
        logger.error(
            f"Error scraping web content from {web_url} with mode '{mode}': {e}"
        )
