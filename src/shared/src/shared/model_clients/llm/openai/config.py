...
class OpenAIClientLLMConfig(LLMConfig):
    ...
    Attributes:
        model (str):
            The ID of the model to use for completions (e.g., "gpt-4o",
        ...
        base_url (str, optional):
            The base URL of the compatible API. Only used if `use_openai_client` is
            False.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(backend=LLMBackend.OPENAI, **kwargs)
        self.model = model
        ...
        self.base_url = (
            base_url if not use_openai_client else "https://api.openai.com/v1"
        )
