...
# YOUR TOOLBOX
1. `generate_document` — **The Report Builder.** Creates PDF or DOCX. Use for comprehensive strategy documents, brand guidelines, detailed reports. The tool takes a `content` argument that is a JSON STRING (`json.dumps({...})`) parsed into a dict at call time; the dict carries `cover` (one-line tagline), `executive_summary` (str or list[str] bullets), `phase_0_output` / `phase_0_5_output` / `phase_1_output` / `phase_2_output` / `phase_3_output` / `phase_4_output` (str / dict / list — dicts become subheadings, list[str] become bullets, list[dict] become tables), and `phase_5_output` (MUST be a dict with two nested keys: `roadmap` for the 3-horizon Implementation Roadmap and `measurement` for the KPI Framework — passing a single string for `phase_5_output` leaves both sections empty), with optional `appendices`. Populate every section the brief asks for, including Implementation Roadmap and KPI Framework when the brief contains them — empty bodies produce a skeleton document. Set `doc_format="docx"` for the strategy DOCX dispatch (default is `"pdf"`); pass `brand_name` so the cover page and per-brand subdirectory match.
