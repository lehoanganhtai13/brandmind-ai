...
def test_crawl4ai_client():
    ...
    for url_info in test_urls:
        ...
        try:
            result = client.fetch_clean_content(
                url_info["url"], Crawl4AIPresets.balanced()
            )
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                f.write("METHOD: clean_content\n")
                f.write(f"URL: {url_info['url']}\n")
                f.write("CONFIG: balanced preset\n")
        ...
        try:
            ...
            filepath = os.path.join(results_dir, filename)
            with open(filepath, "w", encoding="utf-8") as f:
                f.write("METHOD: content_summary\n")
                f.write(f"URL: {url_info['url']}\n")
                f.write("PROVIDER: gemini/gemini-2.5-flash\n")
                f.write(f"CONTENT LENGTH: {content_length} characters\n")
                f.write("=" * 80 + "\n")
                f.write(result.content)
        ...
        try:
            ...
            filepath = os.path.join(results_dir, filename)
            with open(filepath, "w", encoding="utf-8") as f:
                f.write("METHOD: relevant_content\n")
                f.write(f"URL: {url_info['url']}\n")
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Methods Tested: 3 (clean_content, content_summary, relevant_content)\n"
        )
        ...
        for url_name, url_results in all_results.items():
            ...
            for method_name, method_data in url_results.items():
                ...
                if method_data["status"] == "SUCCESS":
                    f.write(
                        f"{method_data['length']:,} chars - {method_data['filename']}\n"
                    )
                else:
        ...
        successful_tests = sum(
            sum(
                1
                for method_data in url_results.values()
                if method_data["status"] == "SUCCESS"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        successful_methods = sum(
            sum(
                1
                for method_data in url_data.values()
                if method_data["status"] == "SUCCESS"
            )
