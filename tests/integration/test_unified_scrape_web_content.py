...
def test_unified_scrape():
    ...
    for url_info in test_urls:
        ...
        try:
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                ...
                f.write(
                    "DESCRIPTION: Clean content extraction with balanced filtering\n"
                )
        ...
        try:
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                ...
                f.write(f"URL: {url_info['url']}\n")
                f.write("DESCRIPTION: LLM-generated structured summary\n")
                f.write(f"CONTENT LENGTH: {content_length} characters\n")
                f.write("=" * 80 + "\n")
                f.write(result.content)
        ...
        try:
            result = scrape_web_content(
                url_info["url"], mode="relevant", query=url_info["query"]
            )
            content_length = len(result.content)
            print(f"   ✅ {content_length:,} characters")
            ...
            filepath = os.path.join(results_dir, filename)
            with open(filepath, "w", encoding="utf-8") as f:
                f.write("MODE: relevant\n")
                f.write(f"URL: {url_info['url']}\n")
                f.write(f"QUERY: {url_info['query']}\n")
                f.write("DESCRIPTION: LLM-filtered content relevant to query\n")
                f.write(f"CONTENT LENGTH: {content_length} characters\n")
                f.write("=" * 80 + "\n")
                f.write(result.content)
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        f.write("Modes Tested: 3 (clean, summary, relevant)\n")
        f.write(f"URLs Tested: {len(test_urls)}\n")
        f.write("Domains Covered: Tech News, Documentation, E-commerce, Blog, News\n\n")
        ...
        for url_name, url_results in all_results.items():
            ...
            for mode_name, mode_data in url_results.items():
                ...
                if mode_data["status"] == "SUCCESS":
                    f.write(
                        f"{mode_data['length']:,} chars - {mode_data['filename']}\n"
                    )
                else:
        ...
        successful_tests = sum(
            sum(
                1
                for mode_data in url_results.values()
                if mode_data["status"] == "SUCCESS"
            )
        ...
        for mode, lengths in mode_stats.items():
            if lengths:
                ...
                success_count = len(lengths)
                total_count = len(test_urls)
                f.write(
                    f"{mode.upper()} mode: {success_count}/{total_count} successful, "
...
if __name__ == "__main__":
    try:
        ...
        successful_modes = sum(
            sum(
                1 for mode_data in url_data.values() if mode_data["status"] == "SUCCESS"
            )
