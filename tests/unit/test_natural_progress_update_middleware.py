"""Tests for model-authored working-note guarding."""
...
from core.brand_strategy.progress_updates import NaturalWorkingNoteMiddleware
...
def test_retries_expensive_first_turn_tool_chain_with_working_note_reminder() -> None:
    """Expensive opening work should be replaced by a working-note tool call."""
    middleware = NaturalWorkingNoteMiddleware()
    request = _request([HumanMessage(content="Làm giúp tôi một proposal đầy đủ.")])
    initial = _response(["task"])
    working_note = _response(["share_working_note"])
    calls: list[ModelRequest] = []

    def handler(model_request: ModelRequest) -> ModelResponse:
        calls.append(model_request)
        return initial if len(calls) == 1 else working_note

    result = middleware.wrap_model_call(request, handler)

    assert result is working_note
    assert len(calls) == 2
    assert any(
        isinstance(message, SystemMessage)
        and "`share_working_note(message=...)` once" in str(message.content)
        and "Use future or intent wording" in str(message.content)
...
def test_does_not_retry_when_model_already_authors_working_note_tool() -> None:
    """A model-authored working-note call is already the desired behavior."""
    middleware = NaturalWorkingNoteMiddleware()
...
async def test_async_retry_matches_sync_behavior() -> None:
    """Async streaming agent calls should get the same working-note guard."""
    middleware = NaturalWorkingNoteMiddleware()
